...
Opcionalmente acepta `"periodo": "YYYY-MM"` para pedir el recibo completo de un periodo anterior (incluye su `desglose` horario en tarifas como GDMTH). Se cobra 1 crédito por (api_key, RPU, periodo) y repetir un periodo ya consultado es gratis para siempre; la profundidad depende de lo que el portal de CFE lista para el servicio — con `micfe` típicamente el historial mensual completo de varios años — y un periodo fuera de esa lista regresa `404` con los disponibles, sin cobrar (con `micfe` requiere también `total_a_pagar` cuando el periodo no está en caché; uno ya consultado se sirve sin él).

**Proveedor de recibos (MiCFE).** El endpoint acepta opcionalmente el proveedor con el campo `"provider"` en el cuerpo o el header `X-CFE-Provider` (valores: `gmx`, `micfe`, `auto`). El proveedor **`micfe`** es la vía soportada de aquí en adelante: en lugar de una búsqueda anónima por RPU + nombre, **enrola el RPU como un "servicio"** en una cuenta de CFE (MiEspacio) y luego descarga su recibo. Enrolar exige un campo adicional **`total_a_pagar`** (string, el monto actual a pagar del recibo, sin decimales) — **requerido cuando el proveedor efectivo es `micfe` y el recibo no está en caché**; un recibo ya en caché se sirve sin él — que CFE valida junto con el `rpu` y el `nombre` (que debe coincidir con la **razón social** registrada por CFE, más estricta que el nombre impreso). Una vez enrolado el servicio, `micfe` sirve tanto el recibo más reciente como los periodos anteriores (`"periodo": "YYYY-MM"`): el portal lista el historial mensual completo del servicio, típicamente varios años hacia atrás.
...
| `400` | `invalid_request`, `total_a_pagar_required`, `name_mismatch`, `total_mismatch`, `enroll_rejected` | RPU malformado, o nombre vacío / inválido (p. ej. el literal `"null"`). Con `micfe`: falta `total_a_pagar` y el recibo no está en caché, o CFE rechaza el enrolamiento porque el nombre (razón social) o el total no coinciden con su registro (el mensaje trae el texto de CFE) |
